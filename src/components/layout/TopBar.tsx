...
import type { Building, Lab, Computer, Alert } from '@/types';
...
export function TopBar({ title, description, onMobileMenu }: TopBarProps) {
  const { currentUser, buildings, labs, computers, alerts } = useApp();
  ...
  const results: SearchResult[] = query.trim().length > 1
    ? [
        ...buildings
          .filter((b) => b.name.toLowerCase().includes(query.toLowerCase()) || b.code.toLowerCase().includes(query.toLowerCase()))
          .map((b: Building) => ({ type: 'building' as const, id: b.id, label: b.name, sublabel: b.code })),
        ...labs
          .filter((l) => l.name.toLowerCase().includes(query.toLowerCase()) || l.code.toLowerCase().includes(query.toLowerCase()))
          .map((l: Lab) => ({ type: 'lab' as const, id: l.id, label: l.name, sublabel: l.code })),
        ...computers
          .filter((c) => c.name.toLowerCase().includes(query.toLowerCase()) || c.assetId.toLowerCase().includes(query.toLowerCase()))
          .map((c: Computer) => ({ type: 'computer' as const, id: c.id, label: c.name, sublabel: c.assetId })),
        ...alerts
          .filter((a) => a.type.toLowerCase().includes(query.toLowerCase()) || a.description.toLowerCase().includes(query.toLowerCase()))
          .map((a: Alert) => ({ type: 'alert' as const, id: a.id, label: a.type, sublabel: a.severity })),
      ].slice(0, 8)
    : [];
